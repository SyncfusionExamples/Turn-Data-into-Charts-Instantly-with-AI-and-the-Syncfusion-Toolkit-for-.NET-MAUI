...
namespace AssistViewMAUI
{
    public abstract class AzureBaseService
    {
        ...
        private void GetAzureOpenAIKernal()
        {
            try
            {
                var client = new AzureOpenAIClient(new Uri(endpoint), new Azure.AzureKeyCredential(key)).AsChatClient(modelId: deploymentName);
                this.client = client;
            }
            catch (Exception)
            {
    ...
    public class AzureAIService : AzureBaseService
    {
        ...
        internal async Task<string> GetResultsFromAI(string relatedChatHistory, string userPrompt, string userAIPrompt)
        {
            if (IsCredentialValid && Client != null)
            {
                try
